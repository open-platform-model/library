...
**Decision**: `schematest` gains `PrivateCacheDir(t) string`: creates `<t.TempDir()>/mod/{extract,download}`, ensures the shared `mod/extract/opmodel.dev` and `mod/download/opmodel.dev` exist, symlinks them, registers a `t.Cleanup` that runs `modcache.RemoveAll` on the private root (registered after `t.TempDir()`, so it runs before `TempDir`'s cleanup, which then finds nothing to do), and returns the root. `registrytest.buildRegistry` and `NewRegistryFromDir` share one `setEnv(t, prefix, host)` helper that sets `CUE_REGISTRY` to the combined mapping and `CUE_CACHE_DIR` to `schematest.PrivateCacheDir(t)` directly; the earlier `schematest.SetEnv(t)` call was dropped because both values it sets were overridden immediately. `SetEnv` and `NewCache` are unchanged for the tests that use the shared cache directly.
...
- [CUE's `modcache` refuses or mishandles a symlinked directory in `extract` or `download` (stat, lock-file creation, temp-file rename into the linked directory)] → the spike task runs a render test warm and cold through the linked tier before the harness changes; the fallback is copying the two subtrees per test (`os.CopyFS`), recorded here if taken, with the cold-cache cost noted above. **Spike result (2026-09-05): the link works.** Warm: the render passed with `opmodel.dev` routed to a dead host, so only the shared cache served core, and the shared cache gained no entries. Cold (`opmodel.dev` subtrees moved aside): core alpha.7 was fetched once into the shared cache through the links (read-only extract directory, zip + mod + lock, no `.partial`, no `.tmp-*`), and the rerun was warm. The `.lock` file persists by CUE's design, as it does for every version in the pre-existing cache. The fallback was not taken.
- [A served-fixture test that also imports a GHCR catalog depending on `cue.dev/x/k8s.io`] → that namespace is outside the linked tier, so it would be fetched into the private cache on every such test. No current `registrytest` consumer does this (the spike's private cache held only `testing.opmodel.dev`); the flow and live tests that pull `cue.dev` build against the shared cache directly. If one appears, add `cue.dev` to the linked namespaces in `PrivateCacheDir` rather than widening the fixture's dependency.
