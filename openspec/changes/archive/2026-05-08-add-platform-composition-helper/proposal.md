...
This is slice 10 of the kernel-redesign umbrella ([001-kernel-redesign-around-platform](../../../enhancements/001-kernel-redesign-around-platform/README.md)). It introduces `pkg/helper/platform/Compose` so every frontend has a one-line composition path.

## What Changes

- Add `pkg/helper/platform/` package with:
...
- `helper-packages`: Adds `pkg/helper/platform/` and its `Compose` function to the helper layer.

## Impact

- **`pkg/helper/platform/` (new)** — `Compose` function and any internal helpers.
- **`pkg/kernel/`** — `(k *Kernel) ComposePlatform` wrapper.
- **`pkg/api/v1alpha2/`** — confirms `Paths().Registry` resolves to the right path for FillPath; may add a small helper for building a `#ModuleRegistration` value.
