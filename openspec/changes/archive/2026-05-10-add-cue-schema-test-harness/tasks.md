...
## 6. Go harness in `pkg/api/v1alpha2/`

- [x] 6.1 Create `pkg/api/v1alpha2/schema_fixture_test.go` with a `schemaCase` struct (fields: `name`, `fixture`, `assertField`, `assertValue`, `expectError`)
...
- [x] 6.5 `go test ./pkg/api/v1alpha2/... -run TestSchemaFixtures -v` — all three subtests pass

## 7. Embed-exclusion regression assertion

- [x] 7.1 Added `TestEmbeddedSchema_ExcludesTestdata` to `pkg/api/v1alpha2/embed_test.go`, asserting no embedded path contains `testdata/`
- [x] 7.2 `go test ./pkg/api/v1alpha2/... -run TestEmbeddedSchema` — green
...
- [x] 8.4 `task test` — `pkg/api/v1alpha2 0.033s ok`; `TestSchemaFixtures` (3 subtests) and `TestEmbeddedSchema_ExcludesTestdata` both green
