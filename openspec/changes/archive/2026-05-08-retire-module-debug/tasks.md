...
- [x] 1.1 `grep -rn "ModuleDebug" pkg/ apis/` and review every hit
...
- [x] 2.1 Inspect `pkg/api/v1alpha2/` (from `add-multi-apiversion-support`); confirm no `DecodeModuleDebugMetadata` or similar exists
...
- [x] 3.2 Update `pkg/module/` godoc to mention `debugValues` is a Module field, not an artifact
...
- [x] 5.3 Run `task lint`
- [x] 5.4 Run `task test`
- [x] 5.5 Final grep: `grep -rn "ModuleDebug" pkg/ apis/ enhancements/ openspec/` returns only the umbrella's expected mentions
