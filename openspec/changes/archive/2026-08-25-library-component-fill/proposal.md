...
- **Rendered output ordering:** 0019 D14's ordering note ships with this slice. Measured on the first run (2026-08-25): once no transformer receives a finalized value, the hoisting of comprehension-produced fields is gone and the kernel's field order equals CUE's declaration order, so the `worker` env-order divergence retired here and the migration note landed in `MIGRATIONS.md` (`## Unreleased — Additive`). The remaining ordering divergence (label maps sorted by the Go-built `#context`) is D12's and stays recorded.
