...
**Who applies the consumer edits:** the two lists above are this change's share of the cost, not a task in this change. They land in the sibling changes `migrate-kernel-api-and-verdicts` in `cli` and in `opm-operator`, each of which also carries `one-api-tier`'s migration — both library changes ship in the same alpha, so neither consumer has a pin at which only one is present.
