...
- [x] 1.1 Define `ValidateInput`, `MatchInput`, `PlanInput`, `CompileInput` in `pkg/kernel/inputs.go` (or equivalent)
...
- [x] 2.1 Define `MatchPlan` (re-export from `pkg/render/`), `PlanResult`, `CompileResult` in `pkg/kernel/results.go`
- [x] 2.2 Add type alias `type ModuleResult = CompileResult` in `pkg/render/` for backward compatibility
...
- [x] 4.1 `git mv pkg/render/process_module.go pkg/render/compile_module.go`
...
- [x] 4.5 Update internal references in `pkg/render/` to use new names
- [x] 4.6 Confirm `pkg/render/module.go` (the per-render execution helper) is unchanged
...
- [x] 6.1 Add `pkg/kernel/phase_test.go` covering `Validate`, `Match`, `Plan`, `Compile` methods against existing fixtures
...
- [x] 7.4 `pkg/kernel/doc.go` package doc gains a section "Phase methods" explaining Validate / Match / Plan / Compile
