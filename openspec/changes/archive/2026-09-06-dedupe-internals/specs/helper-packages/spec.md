...
- **THEN** the second return is a non-nil `*module.Source` in overlay mode whose `Overlay` carries the module's `.cue` files and whose `Root` is the synthetic root the overlay keys sit under
