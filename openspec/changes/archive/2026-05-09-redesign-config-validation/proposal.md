...
- **BREAKING** Add `Source struct { Value cue.Value; Name, Origin string }` and option `Partial() Option` in `pkg/kernel/`. Replaces the deleted `pkg/helper/values/` `Layer` / `Stack` types.
...
- **BREAKING** Delete `pkg/helper/values/` package entirely (`values.go`, `errors.go`, `doc.go`, `KernelOwner` interface, `Layer`, `Stack`, `MultiSourceError`, `LayerError`, `ValidateAndUnify`).
- **BREAKING** Delete custom validation error types from `pkg/errors/`: `ConfigError`, `ValidationError`, `FieldError`, `ErrorLocation`, `GroupedError`, plus `groupCUEErrors`, `GroupedErrorsFromError`, `normalizeCUEPath`, `NewValidationError`. `TransformError` stays (execute-phase, unrelated).
- **BREAKING** Remove `Kernel.ValidateAndUnify` wrapper in `pkg/kernel/wrappers.go` (replaced by `Kernel.ValidateConfigDetailed`).
...
- Internal `walkDisallowed` + `fieldNotAllowedError` stay private inside `pkg/kernel/validate.go`; they implement `cueerrors.Error` and exist only to work around CUE's loss of source positions for closed-schema field rejections.
...
- `helper-packages`: Remove any requirements describing `pkg/helper/values/` (the package is deleted).

## Impact

**Affected library packages**

- `pkg/kernel/`: `validate.go` rewritten; new `source.go`, `source_loader.go`, `print.go`; `wrappers.go` loses `ValidateAndUnify`; `phases.go` and `process.go` updated to call new primitives.
- `pkg/module/`: `module.go` and `release.go` gain `ConfigSchema` accessors and four validation methods each.
- `pkg/helper/values/`: deleted entirely.
- `pkg/errors/`: shrinks to roughly `TransformError` + sentinels. `domain.go` and `config_error.go` lose validation-related types.
...
- `cli/`: every callsite that imports `pkg/helper/values` or constructs `*ConfigError`/`*MultiSourceError`. Switches to `[]kernel.Source` and `cueerrors.Errors`/`Print`.
...
MAJOR. Public types and function signatures in `pkg/kernel/`, `pkg/helper/values/` (full deletion), and `pkg/errors/` change.
