...
## 2. `opm/apiversion` package (PR 2)

- [x] 2.1 Create `opm/apiversion/apiversion.go` declaring `type Version string`, the `V1alpha2` constant, `ErrUnknownAPIVersion` sentinel, and the `Detect(cue.Value) (Version, error)` helper.
- [x] 2.2 Add `opm/apiversion/apiversion_test.go` covering: recognised version, unknown literal, missing field, non-string field — each asserting `errors.Is(err, ErrUnknownAPIVersion)` for failure cases.
- [x] 2.3 Run `task fmt`, `task vet`, `task lint`, `task test`.

## 3. `opm/api` interface and registry (PR 3)

- [x] 3.1 Create `opm/api/api.go` defining `Paths` struct, the lowest-common-denominator `ModuleMetadata`/`ReleaseMetadata`/`ProviderMetadata` structs, the `ReleaseView` interface, and the `Binding` interface.
- [x] 3.2 Create `opm/api/registry.go` (or in same file) with `Register`, `Lookup`, `For` plus the unexported `sync.RWMutex`-guarded map. `Register` panics on duplicate.
- [x] 3.3 Add `opm/api/registry_test.go` covering: register + lookup happy path, lookup miss returns non-nil error, duplicate-register panics, `For` end-to-end.
- [x] 3.4 Add `opm/api/doc.go` documenting the `init()`-registration contract and the panic-on-duplicate behaviour.
- [x] 3.5 Run `task fmt`, `task vet`, `task lint`, `task test`.

## 4. `opm/api/v1alpha2` binding (PR 4)

- [x] 4.1 Create `opm/api/v1alpha2/binding.go` implementing `api.Binding`. `Paths()` returns the path inventory mirroring today's hardcoded values in `opm/render/{match,execute}.go`.
- [x] 4.2 Move the unexported `moduleReleaseContextData` and `componentContextData` structs out of `opm/render/execute.go` into `opm/api/v1alpha2/context.go`. Implement `BuildTransformerContext` using these structs. Keep a temporary alias in `execute.go` so the renderer still compiles.
- [x] 4.3 Implement `DecodeModuleMetadata`, `DecodeReleaseMetadata`, `DecodeProviderMetadata` mirroring the current decode logic in `opm/loader/provider.go` and `opm/module/parse.go`.
- [x] 4.4 Add `opm/api/v1alpha2/init.go` with `init() { api.Register(&binding{}) }`.
- [x] 4.5 Add `opm/api/v1alpha2/binding_test.go` covering: `Paths()` returns expected values; `DecodeReleaseMetadata` round-trips a fixture; `BuildTransformerContext` produces a `cue.Value` with the expected `#moduleReleaseMetadata`, `#componentMetadata`, `#runtimeName` fields.
...
- [x] 5.2 In `opm/loader/release.go`, after `BuildInstance`, call `apiversion.Detect` on the result. Surface the detected version to callers (return signature change is acceptable here — internal-only consumer is `module.ParseModuleRelease`).
- [x] 5.3 In `opm/loader/module.go`, do the same for `LoadModulePackage`.
- [x] 5.4 In `opm/loader/provider.go`, populate `Provider.ApiVersion` after metadata extraction.
- [x] 5.5 In `opm/module/parse.go`, propagate `Module.ApiVersion` into the resulting `*Release`.
...
- [x] 7.1 Replace `injectContext` in `opm/render/execute.go` with a one-line delegation: `ctxVal, warnings, err := binding.BuildTransformerContext(cueCtx, rel, compName, schemaComp, runtimeName); unified = unified.FillPath(binding.Paths().Context, ctxVal)`.
- [x] 7.2 Delete the temporary `moduleReleaseContextData` / `componentContextData` aliases left in `execute.go` from group 4.
- [x] 7.3 Confirm `opm/module/release.Release` exposes the methods required by `api.ReleaseView` (`ReleaseName`, `Namespace`, `ReleaseUUID`, `ModuleFQN`, `ModuleVersion`, `Labels`, `Annotations`); add thin accessors if missing.
...
- [x] 8.2 Add `opm/apiversion/embed.go` (or a small `opm/api/embed.go`) exposing `EmbeddedSchema(version) (fs.FS, error)` keyed off the registered binding.
- [x] 8.3 Wire `opm/api/v1alpha2/binding.go` to expose its embedded FS through the binding (e.g. `Binding.EmbeddedSchema() fs.FS`).
...
- [x] 12.1 Remove `ComponentBlueprints` from `opm/api/api.go` `Paths`; add doc note explaining why blueprints are not walked at render time.
- [x] 12.2 Remove `ComponentBlueprints` entry from `opm/api/v1alpha2/binding.go` `paths` var.
...
- [x] 13.2 Update all caller sites in `opm/module/parse.go`, `opm/render/{module,process_module}.go`, `opm/loader/provider.go`.
- [x] 13.3 Update tests in `opm/render/render_test.go` and `opm/loader/{module,release,provider}_test.go` (struct literals + `Test*Missing*ApiVersion` → `*APIVersion` test names).
...
- [x] 14.3 Confirm `grep -rn '\.ApiVersion\b' opm/` returns zero hits and `grep -rn '\.APIVersion\b' opm/` returns the renamed accesses.
...
- `opm/api/v1alpha2/consts.go` location is correctly version-scoped per ADR-001.
