## 1. opm/module — Release Config Schema Accessor

- [x] 1.1 Add `(r *Release) ConfigSchema() cue.Value` method in `opm/module/release.go`. Return zero `cue.Value` on nil receiver, on `api.Lookup(r.APIVersion)` error, and when the looked-up path does not exist.
...
- [x] 1.3 Add unit tests in `opm/module/release_test.go` covering: schema reachable on a well-formed release, zero value on unregistered binding, zero value on missing `#config` path, nil-receiver safety.

## 2. opm/kernel — Slim Input Structs

- [x] 2.1 Remove `Module *module.Module` field from `MatchInput` in `opm/kernel/inputs.go`. Update godoc on the struct to reflect the new shape.
- [x] 2.2 Remove `Module *module.Module` field from `PlanInput` in `opm/kernel/inputs.go`. Update godoc on the struct.
- [x] 2.3 Remove `Module *module.Module` field from `CompileInput` in `opm/kernel/inputs.go`. Update godoc on the struct.
- [x] 2.4 Confirm `ValidateInput` is unchanged — its `Module` field stays in this slice.

## 3. opm/kernel — Migrate Internal Callers

- [x] 3.1 In `opm/kernel/phases.go`, remove the `if in.Module == nil { ... }` guard from `Plan` and `Compile`.
...
- [x] 3.4 Update any kernel-internal call sites in `opm/kernel/wrappers.go` that constructed `MatchInput` / `PlanInput` / `CompileInput` to match the new shape.

## 4. opm/kernel — Update Tests

- [x] 4.1 In `opm/kernel/phase_test.go`, update `newPhaseFixture` to embed an `#module` block (with `apiVersion`, `metadata`, `#config: { replicas: int & >0, name: string }`) into the `relPkg` literal. Drop the standalone `mod` field from `phaseFixture` once no test reads it.
...
- [x] 4.3 In `opm/kernel/kernel_test.go`, update fixtures and call sites that constructed `CompileInput{Module: ...}` to drop the field. Leave any `ValidateInput` constructions untouched.
