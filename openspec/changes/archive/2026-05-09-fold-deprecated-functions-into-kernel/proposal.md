...
Slices 01–11 set up `Kernel` as the public anchor type but kept the canonical implementation of four functions in their original packages (`opm/validate/`, `opm/module/parse.go`, `opm/compile/compile_module.go`), with `Kernel` methods as thin wrappers carrying `//nolint:staticcheck` exemptions. The wrapper-with-deprecation step was always transitional — this slice closes that arc by moving the implementations into `opm/kernel`, deleting the deprecated free functions and the `opm/validate/` package entirely, so `Kernel` is the *only* place the canonical impls live.
...
- **BREAKING**: `opm/validate/` package deleted in full. Callers (none remain in this repo after the migration) must use `Kernel.ValidateConfig` / `Kernel.ValidateConfigPartial`.
...
- **Added**: `Kernel.ProcessModuleRelease` — canonical name; carries the impl moved out of `opm/module/parse.go`.
...
- **Interface widening**: `opm/helper/values.KernelOwner` gains a `ValidateConfigPartial` method so the helper can call the kernel without re-introducing the package import cycle (`helper/values → kernel → helper/values`).
- **Constitution edit**: `openspec/config.yaml` package list drops `opm/validate/`.
...
- **Affected packages**:
  - `opm/kernel/` — gains `validate.go`, `process.go`, `compile.go` (impl files); `wrappers.go` shrinks (loses validate + parse + compile wrapper methods, retains the helper-shaped wrappers).
  - `opm/validate/` — deleted.
  - `opm/module/parse.go` — deleted.
  - `opm/compile/compile_module.go` — deleted (impl absorbed into `opm/kernel/compile.go`).
  - `opm/helper/values/values.go` — `KernelOwner` interface widens; helper calls `owner.ValidateConfigPartial(...)` instead of `validate.ConfigPartial(...)`.
  - Tests move with their impl: `opm/validate/config_test.go` → `opm/kernel/validate_test.go`; parity-against-deprecated-fn tests in `opm/kernel/kernel_test.go` collapse to single-path tests against the kernel methods.
