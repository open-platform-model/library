...
The wrapper-with-deprecation step was always transitional. This change finishes it: the impls move into `opm/kernel`, the free functions are deleted, and `opm/validate/` (which exists only to host two of those functions) is deleted with the package.
...
The constraint forcing the design: `opm/helper/values` calls `validate.ConfigPartial`. If `validate` becomes a wrapper around `kernel`, the import graph closes a cycle (`helper/values → validate → kernel → helper/values`). The only way to delete `opm/validate/` cleanly is to widen the `KernelOwner` interface in `opm/helper/values` so the helper calls back into the kernel through the interface rather than through a sibling package.

## Goals / Non-Goals

**Goals:**

- Delete `opm/validate/` in full, including tests.
- Delete `opm/module/parse.go` and `opm/compile/compile_module.go`.
- Move the four canonical impls into `opm/kernel/` as kernel-internal files.
...
- Widen `opm/helper/values.KernelOwner` to include `ValidateConfigPartial`.
- Remove all `//nolint:staticcheck // SA1019: …wraps the deprecated free function` exemptions from `opm/kernel`.
- Update `openspec/config.yaml` constitution to drop the `opm/validate/` line from the package-boundary list.
...
- Folding `compile.NewModule` (the deprecated constructor inside `opm/compile/module.go`). That belongs to a different cleanup stream.
- Touching `opm/loader/` (the legacy directory predating `opm/helper/loader/file/`). Out of scope.
...
### Decision 2: Widen `KernelOwner` in `opm/helper/values` rather than re-route via a third shared package

**Choice.** The `KernelOwner` interface in `opm/helper/values/values.go` grows one method:
...
**Rationale.** This is the minimum surface that breaks the cycle. The helper still doesn't import `opm/kernel` directly — it depends only on the interface. `*kernel.Kernel` automatically satisfies the wider interface because the new method already exists on it.
...
### Decision 3: Three impl files under `opm/kernel/` — `validate.go`, `process.go`, `compile.go`

**Choice.** Each canonical impl gets its own file in `opm/kernel/`, named for the operation rather than the source package:

- `opm/kernel/validate.go` — `ValidateConfig`, `ValidateConfigPartial`, plus the unexported helpers (`appendSchemaErrors`, `walkDisallowed`, `fieldNotAllowedError`, `normalizeFieldPath`).
- `opm/kernel/process.go` — `ProcessModuleRelease` (and the unexported `bestEffortReleaseName` helper). `Kernel.ParseModuleRelease` (deprecated alias) lives here too so the rename is co-located.
- `opm/kernel/compile.go` — the `Compile` impl currently in `opm/compile/compile_module.go`. The kernel `Compile` method that exists in `opm/kernel/phases.go` calls into this file's internal helper rather than the deleted `compile.CompileModuleRelease`.

`opm/kernel/wrappers.go` shrinks: it loses `ValidateConfig`, `ValidateConfigPartial` (it never had this one), `ParseModuleRelease`, and any `Compile`-related delegation. It retains the helper-shaped wrappers (`LoadModulePackage`, `LoadReleaseFile`, `LoadValuesFile`, `LoadPlatformFile`, `NewPlatformFromValue`, `ComposePlatform`, `NewModuleFromValue`, `NewReleaseFromValue`, `ValidateAndUnify`) per Non-Goal 1.
...
- *Single `opm/kernel/canonical.go`.* Reads as a dumping ground. Rejected.

### Decision 4: Tests move with their impl

**Choice.** Test files relocate alongside their implementation:

- `opm/validate/config_test.go` → `opm/kernel/validate_test.go` (rewritten to use `kernel.New()` + `k.ValidateConfig` / `k.ValidateConfigPartial`).
- The two `TestKernel_ValidateConfig_Parity*` tests in `opm/kernel/kernel_test.go` collapse to single-path tests against `k.ValidateConfig` (the parity-against-deprecated-fn assertions vanish because the deprecated function is gone).
- `TestKernel_ParseModuleRelease_Parity` in `opm/kernel/kernel_test.go` likewise collapses; an additional small test confirms `Kernel.ParseModuleRelease` (the deprecated alias) and `Kernel.ProcessModuleRelease` produce identical results.
