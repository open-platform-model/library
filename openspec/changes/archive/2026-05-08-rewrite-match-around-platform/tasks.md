...
- [x] 2.1 Refactor `opm/render/match.go` to walk `Platform.#matchers` via `binding.Paths().Matchers` instead of iterating a Provider's transformer list
...
- [x] 2.6 Remove every reference to `*provider.Provider` from `opm/render/match.go`

## 3. Execute Phase Rewrite

- [x] 3.1 Update `opm/render/execute.go` to resolve transformers by FQN from `Platform.#composedTransformers` via `binding.Paths().ComposedTransformers`
- [x] 3.2 Remove every reference to `*provider.Provider` from `opm/render/execute.go`
- [x] 3.3 Confirm FillPath / decode / emit behavior is otherwise unchanged

## 4. Runtime Helper Update

- [x] 4.1 Update `render.Module` (in `opm/render/module.go`) to hold `*platform.Platform` instead of `*provider.Provider`
...
- [x] 7.1 Delete `opm/provider/` directory entirely
- [x] 7.2 Delete `opm/helper/loader/file/provider.go`
- [x] 7.3 Remove `LoadProvider` re-export from the `opm/loader/` deprecation shim
- [x] 7.4 `grep -rn "provider.Provider" opm/` and confirm zero hits
- [x] 7.5 `grep -rn "LoadProvider" opm/` and confirm zero hits
...
- [x] 9.4 `opm/render/match.go` package doc: describe the algorithm clearly; cross-reference catalog 014's `#PlatformMatch` as the conceptual source
