...
- **BREAKING** to `opm/module/` exported types — consumers reading `.Spec` / `.Config` / `.Values` directly must migrate to `Package.LookupPath(...)` via the binding.
...
- This is a MAJOR change for `opm/module/`. Bump the kernel module version accordingly when this lands.
...
- **`opm/module/`** — `Module` and `Release` types refactored. Constructor helpers added (`NewModuleFromValue`, `NewReleaseFromValue`).
- **`opm/loader/`** — module / release loading produces typed values via the new constructors. `loader.LoadModulePackage` continues to return raw `cue.Value`; a new helper builds `*Module` from it.
- **`opm/render/`, `opm/validate/`** — internal call sites that read `mod.Spec`, `mod.Config`, `rel.Values`, `rel.Module.Config` migrate to `Package.LookupPath` via the binding (from `add-multi-apiversion-support`).
- **`opm/kernel/`** — Kernel wrapper methods (slice 01) update return types where applicable.
