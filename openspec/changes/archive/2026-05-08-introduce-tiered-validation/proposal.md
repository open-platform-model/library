...
- Remove `validate.UnifyAndValidate` (deprecated in slice 04) — frontends migrate to `opm/helper/values`.
- This is a MINOR change to the public API. `opm/helper/values/` is new; `validate.UnifyAndValidate` removal is the only deletion.
...
- **`opm/helper/values/` (new)** — `Layer`, `Stack`, `ValidateAndUnify`, `MultiSourceError`.
- **`opm/validate/`** — `UnifyAndValidate` is removed. `Config` is unchanged from slice 04.
- **`opm/kernel/`** — optionally adds a thin convenience method `(k *Kernel) ValidateAndUnify(schema, layers)` delegating to the helper.
