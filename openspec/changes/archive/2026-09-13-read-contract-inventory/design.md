...
**As landed (task 3.2)**: the inventory enters `#Match` as a third input, `#definedBy: [string]: string`, and the `match:` site passes `platform.#contracts.definedBy`, so `#Match` stays parameterised by its inputs rather than reaching for the `platform` import from inside; the rows read `#definedBy[_f]` under the same `!= _|_` guard. When alternatives exist and a catalog lists the key, `describe()` names both ("defined by X, implemented at a different apiVersion"), a fourth wording that is the alternatives case with the catalog beside it, not a fourth verdict. The fixture set gained `scenarios/unlisted` (an inline contract no catalog lists: the third case) and `platform_disabled` (cat disabled beside cat2 0.1.0: the "disabled catalog defines nothing" scenario), since every member of `cat` is now listed and no existing fixture could produce an empty `DefinedBy`.
...
**Measured (task 2.1, on the listed `platform` fixture acquired through `newRenderKernel`)**: `Package.LookupPath(cue.MakePath(cue.Def("contracts")))` exists and each of the six data fields decodes by path. One `Decode` over the whole `#contracts` value into a Go struct that omits `defined` also works (CUE's `Decode` reads only the fields the Go type names) while `defined` sits beside them non-concrete (`Validate(cue.Concrete(true))` on it reports the members' required `spec` fields). The per-path form is kept anyway: the decoded set is the type's field list by construction rather than by `Decode`'s field skipping, and a missing field is named in the error. The fixture values pinned: `platform` defines 17 contracts, all by `testing.opmodel.dev/library-render/cat@v0`; `container@v1` is required by the deployment and service transformers; `orphan@v1`, `ladder@v2`, `backup@v1`, `sidecar@v1` and `unstated@v1` by nothing; `gateway@v1` (the one provider-fulfilled member) by `gateway-transformer` alone, so `Unfulfilled` and `OverSubscribed` are empty and both booleans true. `platform_oversubscribed` adds cat2 0.2.0's `mirror-transformer` to `container@v1` and its `gateway-transformer` to `gateway@v1`: `OverSubscribed` is `[gateway@v1]`, `Routable` false, `Fulfilled` true. cat2 lists nothing (it defines no member of its own), so a platform embedding cat2 alone derives the empty inventory.
