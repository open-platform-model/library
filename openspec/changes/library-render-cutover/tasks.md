...
- [x] 2.1 `opm/schema/loader.go`: `DefaultSchemaModule` to `opmodel.dev/core@v2.0.0-alpha.7`; update its test and the doc comment; run the workspace `task deps:update` for `library` only (the deferred bump) and commit its `cue.mod` diffs.
...
- [x] 2.9 `task check` green under `-race`; `go vet ./...` shows no dangling references; grep `cli/` and `opm-operator/` for the removed identifiers and list every hit in the PR description for the switch changes.
