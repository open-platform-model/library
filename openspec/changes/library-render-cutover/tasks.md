...
- [x] 3.1 `adr/005-shares-nothing-renders.md` per design.md (two rules, rejected alternatives with measurements, pool sizing by memory); ADR-002 status "Superseded by ADR-005"; ADR-003 status paragraph retiring the federation premise.
  ...
  - [x] 3.1.2 ADR-003 status paragraph scopes out its federation application (`indexCatalogs`, `MaterializedPlatform.Transformers` / `.Matchers`, "read the materialized index off ..."): the no-cross-build-fill principle stands, the materialize application is deleted.
