...
- [ ] 1.2 Record in ADR-009's Consequences that `cli/internal/publish/compat.go` becomes a collapse candidate and that `loader.FetchModule`'s name is now kind-agnostic in fact but not in spelling. Verify: both are named as consequences, neither as work this change does.
