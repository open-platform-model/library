...
- **CUE schema (`apis/core/`)** — Library's embedded copy is re-synced from the new `core` repo: flat layout (no `v1alpha2/` subdir), package renamed from `v1alpha2` to `core`, `apiVersion: #ApiVersion` removed from every artifact root, `#ApiVersion` constant dropped, `cue.mod/module.cue` retargeted to match the new core repo's module identity.
