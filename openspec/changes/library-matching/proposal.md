...
- **The always-unify rung compares provenance and would fire on every catalog release.** D26/D30: the unify comparison must exclude exactly `metadata.catalogVersion` and `metadata.description` from both operands (definition and instance) while keeping the closed definition in the comparison — the strip mechanism shipped in `library-compat-comparator` (`compat.StripProvenance`); this change applies it at the rung.
...
- **`opm/compile/match.go`:**
  ...
  - The unify rung strips both operands with `compat.StripProvenance` before `Unify(...).Validate(cue.Concrete(false))`, keeping closed definitions in the comparison. Recorded cost (D30): CUE messages lose document position; `UnifyError`'s structural fields already carry component and FQN.
