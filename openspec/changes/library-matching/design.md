...
### The unify rung's strip (D26/D30)

**Context**: `unifyIntersection` unifies the component's `#resources[fqn]`/`#traits[fqn]` against the transformer's `required*[fqn]` — both sides carry `metadata.catalogVersion`, which diverges across builds by construction. D30 specifies the denylist strip; `compat.StripProvenance` ships it.
**Decision**: Strip both operands. The component-side value is stripped **once per component** (before the candidate loop); the required-side value is stripped **once per transformer FQN**, memoised beside the existing unify-verdict cache — the strip is a syntax round-trip and per-pair stripping would redo it for every candidate against the same component.
**Scope of "every metadata block"**: literal, per D30's text — the AST walk deletes the two named fields from every `metadata` block it finds. A nested `spec.podMetadata`-style block cannot legally carry `catalogVersion`/`description` as *those* fields (they are primitive-metadata vocabulary), so the literal rule is safe; if a future schema puts a user field named `catalogVersion` under a nested `metadata`, the strip eats it — accepted and documented, matching the decision text rather than second-guessing it.
**Closedness**: the strip preserves closedness (pinned by `compat`'s tests); the rung keeps unifying the closed definition, which is what makes D27 enforceable at match time (cutting at `spec` was measured to silently drop module-set fields).
**Recorded cost**: post-strip CUE errors carry no document position; `UnifyError.{Component, FQN}` carry the structural facts. The rung's error text changes shape (`_#def.…: field not allowed`) — the MIGRATIONS entry names this.
...
- Unify-rung error text loses document position (D30 cost); route on `UnifyError` fields, not message patterns.
