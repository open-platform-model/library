...
- **No behaviour change for previously-valid inputs.** The matching and identity-read code paths are untouched; those are the downstream slices. One discovered exception in subscription resolution: a v2 `#registry` key carries its catalog's `@vN` major (required by v2's `#ModulePathType`), which the pull path's load-ID composition could not express — `pullCatalog` now splits the suffix off before composing the load ID. Major-free (v1-era) keys pass through unchanged, so no previously-expressible input changes behaviour. Two transitional invariants cover the rest (see design, incl. "Subscription-key major handling at pull").
