...
## Purpose
Defines the single-schema dispatch surface that replaces the retired multi-`apiVersion` binding registry. The library vendors exactly one OPM CUE schema under `library/apis/core/` and exposes its paths, metadata decoders, transformer-context builder, and cached schema loader as free functions and package-level variables in `opm/schema`. Callers no longer detect schema versions, look up bindings, or carry `APIVersion` on artifact structs.
