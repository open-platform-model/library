# platform-synthesis Specification

## Purpose

The `opm/helper/synth/` package produces a `#Platform` artifact CUE value from in-memory typed inputs, as the platform-scoped peer of `synth.Release`. Where `synth.Release` synthesizes a `#ModuleRelease`, `synth.Platform` synthesizes a pre-materialize `#Platform`: it unifies caller-supplied identity and subscription fields against the `#Platform` definition resolved through a caller-supplied `*schema.Cache`, leaving the kernel-filled materialization slots unset. The kernel exposes `SynthesizePlatform` as the recommended entry point, chaining synthesis into a typed `*platform.Platform` without performing any registry I/O — materialization remains a separate, explicit, caller-driven step.
