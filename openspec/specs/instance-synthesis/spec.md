...
<!-- Synced from simplify-render-single-build as MODIFIED, but it had no matching requirement name in this spec; appended as new. Review whether it should supersede parts of "synth.Instance function signature" / "Derived fields come from schema unification". -->

### Requirement: synth.Instance constructs the instance by single-build CUE evaluation

`synth.Instance` SHALL construct the `#ModuleInstance` value by synthesizing an in-memory CUE package and evaluating it in a **single build** (per ADR-003), through the same loader build path used by `LoadInstancePackage`. The synthesized package SHALL consist of a fabricated `cue.mod/module.cue` declaring the dependencies (the resolved `opmodel.dev/core` version and the module's `path@version`), an instance source that **imports** the module and writes `#module: <import>` plus caller-supplied `metadata`, and a values source rendered from `InstanceInput.Values`.
...
- **WHEN** `synth.Instance` is called with a module whose identity is author-supplied (`metadata.modulePath` / `metadata.version` concrete) against a resolved `core` schema exposing author-supplied `#Module` identity
