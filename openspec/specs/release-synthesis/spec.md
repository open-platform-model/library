...
The `pkg/helper/synth/` package produces OPM artifact CUE values from in-memory typed inputs. It is the in-memory counterpart to the file-based loader: where `pkg/helper/loader/` parses artifacts from byte streams, `synth` synthesizes them by unifying typed Go inputs against the embedded version-specific schema. The package exposes `synth.Release` as its first artifact builder, returning a `#ModuleRelease` `cue.Value` ready for downstream validation, concreteness enforcement, and metadata decoding via `Kernel.ProcessModuleRelease`.
