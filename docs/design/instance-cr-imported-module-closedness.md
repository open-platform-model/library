# Instance CR: imported published module fails to load on `core@v0.4.0` (FIXED — released as `core@v0.5.0`)
...
(`#module: <registry import>`). Does **not** affect `ModuleInstance`, and does
**not** affect `Instance` packages that author the module inline.
...
The operator (and the CLI) can author a `Instance` two ways:

- **inline** — `instance.cue` declares the whole `#Module` body itself
  (`#module: { core.#Module, metadata: …, #components: … }`). One `#Module`
  instance in one evaluation; nothing is re-unified. **Works.**
- **imported** — `instance.cue` imports a *published* module and binds it
  (`#module: webapp`). This re-unifies an already-closed, published `#Module`
  into `#ModuleInstance.#module!: #Module`. On `core@v0.4.0` this **failed**:
...
CR), so "Instance works" is only half true on the published schema — the inline
...
A scratch module pinning `opmodel.dev/core@v1` + a published module
(`opmodel.dev/modules/web_app@v0.0.1`), with `instance.cue`:
...
already-closed published `#Module` into `#ModuleInstance.#module!: #Module`
...
### Why `ModuleInstance` was never affected

`ModuleInstance` never re-unifies a published module in CUE. The controller
calls `synth.Instance` (`library/opm/helper/synth/release.go`), which sidesteps
...
`Instance` load path (`helper/loader/file.LoadInstancePackage`) has no equivalent
escape hatch: it evaluates the author's `instance.cue` verbatim, which performs
...
it also makes `synth.Instance`'s `userModule`-scope workaround droppable for any
...
1. **Bump stale consumers off `v0.4.0`.** — **DONE.** `opm-kind-demo`'s
   `web_app/cue.mod/module.cue` pins `opmodel.dev/core@v1` → `v0.5.0`; no live
   `v0.4.0` pin remains in the demo.
2. **Drop the `synth.Instance` workaround** — **TRACKED** in
   `simplify-render-single-build` (tasks 3.1–3.3: rewrite `Instance` onto a
   single-build virtual package, delete `buildInstanceScope` / the `userModule`
   `cue.Scope`, and delete the `FillPath(#config)` Go pre-merge). The fix goes
   beyond "let `ModuleInstance` re-unify directly" — it merges both paths onto one
   mechanism. The workaround + its now-stale `v0.4.0` comment in
   `opm/helper/synth/release.go` remain until that change lands.
3. **Add `LoadInstancePackage` coverage** — **TRACKED** in
   `simplify-render-single-build` (task 4.7: direct `LoadInstancePackage` import
...
4. **Correct stale claims** — **DONE.** `opm-kind-demo/web_app/instance.cue` +
...
- `library/opm/helper/synth/instance.go` — `synth.Instance` now constructs the instance via single-build CUE evaluation of a synthesized package that **imports** the module; the `userModule`-scope workaround and the Go `#config` pre-merge were deleted by `simplify-render-single-build`.
- `library/opm/helper/loader/file/build.go` — the shared `buildAndShapeGate` both `synth.Instance` (overlay source) and `LoadInstancePackage` (on-disk source) now run.
- `library/opm/helper/loader/file/instance.go` — `LoadInstancePackage` (re-pointed at the shared build step).
