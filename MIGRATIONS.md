...
This file records the pre-release evolution of the library's public Go API and the OPM schema it ships: every breaking change with a worked migration recipe, staged under `Unreleased — next MAJOR` / `Unreleased — next MINOR`.

From the first tagged release onward, released versions are summarised in [CHANGELOG.md](./CHANGELOG.md) (generated by release-please from Conventional Commits). This file remains the home for detailed breaking-change migration recipes.
