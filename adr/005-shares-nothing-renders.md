...
**Relation to ADR-006:** this ADR governs the lifetime and concurrency of the render build; ADR-006 (superseding ADR-003) governs its construction, one CUE build that imports its inputs with no cross-build fill. The two are one design: the single build is what makes dropping the context per render affordable, and dropping it is what makes the single build safe to run concurrently.
