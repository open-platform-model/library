...
5. **Provider concept is being retired.** Catalog enhancements 014 (`#Platform`) and 015 (`#Claim`) replace the flat `#Provider` artifact with a `#Platform` register that holds `#Module` registrations and computes `#composedTransformers` / `#matchers` from them. Today's `pkg/render/match.go` walks a Provider's transformer list directly; that walk is structurally incompatible with the new design.
...
- Match phase rewritten to consume `Platform.#composedTransformers` + `Platform.#matchers` from catalog 014. `#Provider` retired in lockstep with the catalog.
...
- `#Claim` support in the kernel — deferred until catalog 015 stabilizes. The matcher rewrite (slice 09) covers Resources/Traits only; Claims fold in later.
