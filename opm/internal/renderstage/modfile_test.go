...
`)
	...
	assert.Equal(t, Dep{Version: "v2.0.0-alpha.10"}, local.Deps["opmodel.dev/core@v2"])
...
func TestPromote_PlatformWinsSharedPath(t *testing.T) {
	...
	assert.Equal(t, "v2.0.0-alpha.10", p.Deps["opmodel.dev/core@v2"].Version)
...
`, "i")
	...
	require.Equal(t, []VersionRow{
		...
		{Path: "opmodel.dev/core@v2", ModuleVersion: "v2.0.0-alpha.6", PlatformVersion: "v2.0.0-alpha.10"},
...
`, "p")
	...
	assert.Equal(t, []VersionRow{
		...
		{Path: "opmodel.dev/core@v2", ModuleVersion: "v2.0.0-alpha.10", PlatformVersion: ""},
	}, rows)
}
