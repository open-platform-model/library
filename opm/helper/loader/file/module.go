...
// the raw cue.Value. Used by callers (e.g. opm module vet) to load a module
// for validation or rendering.
//
...
// Kernel.
func LoadModulePackage(ctx *cue.Context, dirPath string, opts LoadOptions) (cue.Value, error) {
	...
	if err := shapeGate(val, moduleSpec); err != nil {
		return cue.Value{}, fmt.Errorf("validating module package in %s: %w", absDir, err)
	}
