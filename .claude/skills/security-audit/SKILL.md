...
- The `cue.mod/local-module.cue` replacements the staging writes point only at the two staged input trees and, only when the frontend opted in through `RenderInput.LocalReplacements`, at the directories (or module paths) an input's own `cue.mod/local-module.cue` names; with the opt-in off, an input carrying such a file is refused before staging, never silently dropped. Under the opt-in the platform's replacements are promoted whole and the instance's only for paths the platform's dependency list does not name, so an untrusted instance module cannot swap the bytes of a path the platform pins
